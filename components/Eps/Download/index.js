    ...
    <div className={`${hidden} ${responsif}`}>
      ...
      <div className="flex pt-10 gap-5 justify-around">
        ...
        <div className={`basis-1/4 text-center bg-slate-900 cursor-pointer ${datazz5 == 2 ? "" : "h-fit"}`}>
          <p className="text-base p-2 max-[765px]:text-sm max-[765px]:p-1" onClick={()=>{datazz5 == 2 ? setDatazz5(0) : setDatazz5(2)}}>480p</p>
          {datazz3 &&
            datazz3.map((item, i) => (
        ...
        <div className={`basis-1/4 text-center bg-slate-800 cursor-pointer ${datazz5 == 3 ? "" : "h-fit"}`}>
          <p className="text-base p-2 max-[765px]:text-sm max-[765px]:p-1" onClick={()=>{datazz5 == 3 ? setDatazz5(0) : setDatazz5(3)}}>1080p</p>
          {datazz4 &&
            datazz4.map((item, i) => (
              <div key={i} className={`py-2 text-xs ${i % 2 ? "bg-slate-800" : "bg-slate-900"}  ${datazz5 == 3 ? "" : "hidden"}`}>
                <Link className="" href={item.link}>{item.host}</Link>
              </div>
            ))}
        </div>
      </div>
