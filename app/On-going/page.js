...
export default function Ongoing() {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        console.log(res.data.ongoing.length);
        data2 == null ? setData2(res.data.ongoing) : setData2([...data2, ...res.data.ongoing]);
  ...
  return (
    <div className="pt-10 pb-5 px-7 max-[765px]:px-3 max-[765px]:pt-10">
      <div className={`max-[765px]:px-0`}>
        <div className="text-xl pb-10 font-semibold tracking-[5px]">
          <Link href={`/Ongoing`}>{`Ongoing Anime`}</Link>
        </div>
